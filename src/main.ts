...
(async function () {
  ...
  themeSelectElement!.innerHTML = themes
    .map((theme: any) => {
      return `<option value='${theme.name}'>${theme.displayName}</option>`;
    })
    .join('');

  themeSelectElement!.addEventListener('change', () => {
    const themeName = (themeSelectElement! as any).value as string;

    loadTheme(
      themes.find((theme: any) => theme.name === themeName) || themes[0]
    );
  });
